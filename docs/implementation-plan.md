...
2. Shell orchestrator runtime in `src/core/orchestrater/`
...
The orchestrator is shell-first because process control, command execution, and build validation are naturally handled in shell. Structured tasks such as JSON parsing, state aggregation, lock management, and log extraction are delegated to Kotlin because they are brittle in shell and should share the same language ecosystem familiar to mobile engineers.
...
The spelling `src/core/orchestrater/` is intentionally preserved for compatibility with the original mission requirements.
...
2. Load `.mobile-workshop/config.json` if present
3. Autodetect repo structure when config is absent
4. Build or load an execution graph
5. Acquire the run lock
6. Initialize run state
7. Execute the selected agent lanes
8. Run validation gates
9. Offer targeted self-heal retries on failures
10. Aggregate metrics and archive the run summary
...
During concurrent phases, the orchestrator uses cooperative polling at explicit intervals. V1 does not use OS-level watchers or background daemons.

## State and Locking Model

Runtime state lives inside the target repository:

- `.context/state/lock.json`
...
All structured writes are handled through the Kotlin helper using atomic temp-file writes followed by `mv`.
...
- lock acquisition, release, and stale-lock repair
...
- Implement Kotlin helper commands for config, state, locks, metrics, and error extraction
- Implement shell libraries for process orchestration and host detection

### Phase 3a

- Implement deterministic routing and file-journal baseline execution
...
- Verify `doctor --repair` clears only stale locks
...
- `src/core/orchestrater/` keeps the requested spelling
